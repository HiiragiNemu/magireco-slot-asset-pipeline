#!/usr/bin/env python3
"""Build the bounded P17 v71r2 human-review package.
...
SCHEMA = "magireco-p17-replacement-review-plan-v1"
EDITIONS = ("none", "ja", "zh")
...
def load_contract(plan_path: Path, ffprobe: str) -> tuple[dict, Path, list[dict]]:
    ...
    if plan.get("schema") != SCHEMA:
        raise ValueError("P17 review plan schema differs")
    if plan.get("family") != "ac6004" or plan.get("publishable") is not False:
        raise ValueError("P17 review plan identity/readiness differs")
    if plan.get("review_status") != "HUMAN_PLAYBACK_REQUIRED":
        raise ValueError("P17 review plan must remain human-playback-required")
    ...
    if (
        ...
        or manifest.get("family") != "ac6004"
        or manifest.get("human_review_status") != "pending"
        or manifest.get("publishable") is not False
        or qa.get("status") != "passed"
        or qa.get("family") != "ac6004"
    ):
        raise ValueError("P17 source manifest/QA state differs")
    if manifest.get("audio_profile") != "no_bgm":
        raise ValueError("P17 source audio profile differs")
...
def verify_output(plan_path: Path, output_root: Path, ffprobe: str) -> dict:
    plan, source_manifest_path, source_rows = load_contract(plan_path, ffprobe)
    index_path = output_root / "manifests" / "REVIEW_INDEX.csv"
    if not index_path.is_file():
        raise ValueError("P17 review index is missing")
    ...
    if [row.get("edition") for row in index_rows] != list(EDITIONS):
        raise ValueError("P17 review index edition order differs")
    copied_plan = output_root / "manifests" / "SOURCE_PLAN.json"
    copied_manifest = output_root / "manifests" / "SOURCE_FAMILY_MANIFEST.json"
    if file_sha256(copied_plan) != file_sha256(plan_path):
        raise ValueError("P17 packaged source plan differs")
    if file_sha256(copied_manifest) != file_sha256(source_manifest_path):
        raise ValueError("P17 packaged source family manifest differs")
    verified = []
    for source_row, index_row in zip(source_rows, index_rows):
        ...
        if index_row.get("human_status") != "HUMAN_PLAYBACK_REQUIRED":
            raise ValueError("review index promoted P17 without owner playback")
    ...
    return {
        "schema": "magireco-p17-replacement-review-verification-v1",
        "result": "PASS",
...
def build(plan_path: Path, output_root: Path, ffprobe: str) -> None:
    ...
    try:
        ...
        lines = [
            ...
            "",
            "## 建议顺序",
            "1. 先看 `ZH/story/P17_八千代等人拯救谣鹤乃_ac6004__zh.mp4`。",
        ...
        lines.extend(
            [
                ...
                "- 旧 P17 隔离文件继续隔离；本目录不会自动替换或晋升它们。",
                "- 失败的 v71（缩短 ac6004_008）未进入本目录。",
                "- P16、P18 未进入本目录。",
                "",
            ]
        )
