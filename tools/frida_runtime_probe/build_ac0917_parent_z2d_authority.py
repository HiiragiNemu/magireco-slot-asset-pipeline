"""Extract every exact parent Z2D referenced by the ac0917 runtime scene tree."""
...
EXPECTED_Z2D_OCCURRENCE_COUNT = 35
EXPECTED_UNIQUE_Z2D_COUNT = 28
EXPECTED_EXTRACTABLE_Z2D_COUNT = 23
...
def collect_event_z2d_bindings(
    ...
    if not isinstance(events, dict) or len(events) != EXPECTED_EVENT_COUNT:
        raise AuthorityError(
            f"ac0917 runtime event count differs: {len(events or {})}"
        )
    foreign_events = sorted(
        event_name for event_name in events if not event_name.startswith("ac0917_")
    )
    if foreign_events:
        raise AuthorityError(f"foreign runtime event: {foreign_events[0]}")
    actual_events = set(events)
    if actual_events != EXPECTED_EVENT_NAMES:
        missing = sorted(EXPECTED_EVENT_NAMES - actual_events)
        extra = sorted(actual_events - EXPECTED_EVENT_NAMES)
        raise AuthorityError(
            f"ac0917 runtime event set differs: missing={missing}, extra={extra}"
        )
    ...
    if len(all_names) != EXPECTED_Z2D_OCCURRENCE_COUNT:
        raise AuthorityError(
            f"ac0917 parent Z2D occurrence count differs: {len(all_names)}"
        )
    unique_names = sorted(set(all_names))
    if len(unique_names) != EXPECTED_UNIQUE_Z2D_COUNT:
        raise AuthorityError(
            f"ac0917 unique parent Z2D count differs: {len(unique_names)}"
        )
    ...
    *, runtime_path: Path, apk: Path, binary: Path, output_dir: Path
    ...
    try:
        ...
        if len(extractable_names) != EXPECTED_EXTRACTABLE_Z2D_COUNT:
            raise AuthorityError(
                "ac0917 exact archive-backed parent count differs: "
                f"{len(extractable_names)}"
            )
        if len(runtime_only_names) != EXPECTED_RUNTIME_ONLY_NODE_COUNT:
            raise AuthorityError(
                "ac0917 runtime-only type-20 node count differs: "
        ...
        report = {
            "schema": "magireco-ac0917-parent-z2d-event-bindings-v1",
            "status": "PASSED",
        ...
        (stage / "AC0917_PARENT_Z2D_EVENT_BINDINGS.json").write_text(
            json.dumps(report, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        (stage / "README.md").write_text(
            "# ac0917 exact parent Z2D authority\n\n"
            "12 runtime event containers reference 35 type-20 occurrences: "
            "23 unique exact APK Z2D chunks and 5 runtime-only numeric nodes. "
