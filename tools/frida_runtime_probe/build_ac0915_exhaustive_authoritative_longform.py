#!/usr/bin/env python3
"""Render the duplicate-free exhaustive native-416 ac0915 no-BGM longform."""
...
EXPECTED_FRAMES = 3933
EXPECTED_EVENTS = 18
EXPECTED_ROUTES = 42
EXPECTED_ROUTE_OCCURRENCES = 177
EXPECTED_LAYER_OCCURRENCES = 46
EXPECTED_UNIQUE_CRI_SOURCES = 42
EXPECTED_UNREACHABLE_LAYERS = 6
EXPECTED_RETAINED_AUDIO = 34
EXPECTED_SUBTITLES = 15
EXPECTED_NORMAL_LAYERS = 44
EXPECTED_ADDITIVE_LAYERS = 2
EDITIONS = {"none": "NONE", "ja": "JP", "zh": "ZH"}
TITLE = "Magius三人全告知胜利特别画面穷尽合集"
CHAPTER_TITLES = {
...
EXPECTED_VISUAL_FRAMES = {**EXPECTED_PRESENTATION_FRAMES, "ac0915_011": 96}
...
class Ac0915LongformBuildError(ValueError):
    pass
...
def validate_authorities(
    ...
    if (
        route.get("schema")
        != "magireco-ac0915-dirinfo-route-and-complete-presentation-dedup-authority-v1"
        or route.get("status") != "PASS_READY_FOR_DUPLICATE_FREE_LONGFORM_RENDER"
    ):
        raise Ac0915LongformBuildError("ac0915 route/dedup authority differs")
    ...
    if any(route_summary.get(key) != value for key, value in expected_route.items()):
        raise Ac0915LongformBuildError("ac0915 route/dedup dimensions differ")
    if (
        ...
        raise Ac0915LongformBuildError("ac0915 projection authority differs")
    if (
        audio.get("schema")
        ...
        raise Ac0915LongformBuildError("ac0915 audio authority differs")
    ...
    if (
        ...
        raise Ac0915LongformBuildError("ac0915 duplicate-free editorial order differs")
    if route.get("dedup_contract", {}).get("identical_alias_events") != {
        "ac0915_008": "ac0915_003",
        "ac0915_019": "ac0915_005",
        "ac0915_021": "ac0915_007",
    }:
        raise Ac0915LongformBuildError("ac0915 complete-presentation aliases differ")
    return order
...
def validate_ida_choice_evidence(path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise Ac0915LongformBuildError(f"IDA runtime-choice evidence differs: {missing}")
    return {
        ...
        "decision": (
            ...
            "identities; the longform enumerates all 42 unique loadable CRI videos once through "
...
def build_event_manifests(
    ...
    for event in order:
        ...
        if not layers:
            raise Ac0915LongformBuildError(f"event lacks visual composition: {event}")
        if event == "ac0915_009" and retained:
            raise Ac0915LongformBuildError("ac0915_009 is expected to be silent")
        if event != "ac0915_009" and not retained:
            raise Ac0915LongformBuildError(f"event lacks retained audio: {event}")
        if any(item["volume_bus"] not in {"SE", "VOICE"} for item in retained):
            raise Ac0915LongformBuildError(f"non-retained bus leaked into {event}")
        frames = int(row["presentation_frame_count"])
        if frames != EXPECTED_PRESENTATION_FRAMES[event]:
            raise Ac0915LongformBuildError(f"event presentation extent differs: {event}")
        visual_frames = max(
            int(item["event_end_frame_inclusive"]) for item in layers
        ) + 1
        if visual_frames != EXPECTED_VISUAL_FRAMES[event] or visual_frames > frames:
            raise Ac0915LongformBuildError(f"visual extent differs: {event}")
        manifests[event] = {
            "event": event,
    ...
    if (
        ...
        raise Ac0915LongformBuildError("canonical production dimensions differ")
    return manifests
...
def layer_filter_parts(
    ...
    if (
        ...
        raise Ac0915LongformBuildError("layer/source frame interval differs")
    x, y, width, height = (int(value) for value in row["output_rect_xywh"])
    if (x, y, width, height) != (0, 0, 416, 232):
        raise Ac0915LongformBuildError("ac0915 output projection differs")
    ...
    if state == 1:
        ...
        if stop_frames < 0:
            raise Ac0915LongformBuildError("additive layer exceeds event")
        parts += [
        ...
        raise Ac0915LongformBuildError(f"unsupported renderer state: {state}")
...
def render_event(ffmpeg: str, manifest: Mapping[str, Any], output: Path, log: Path) -> None:
    ...
    for row in layers:
        ...
        if not path.is_file() or file_sha256(path) != str(row["source"]["sha256"]):
            raise Ac0915LongformBuildError(f"exact CRI source differs: {event}/{path}")
        command += ["-i", str(path)]
    for row in retained:
        path = Path(str(row["official_source"]["path"]))
        if not path.is_file() or path.stat().st_size != int(row["official_source"]["size_bytes"]):
            raise Ac0915LongformBuildError(f"official audio differs: {event}/{path}")
        command += ["-i", str(path)]
    parts = [
...
def write_chapters(
    ...
    if cursor != EXPECTED_FRAMES:
        raise Ac0915LongformBuildError("chapter frame total differs")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return chapters
...
def global_subtitle_cues(
    ...
    if language not in {"ja", "zh"}:
        raise Ac0915LongformBuildError(f"unsupported subtitle language: {language}")
    rows = []
    offset_frames = 0
    for event in order:
        ...
        for cue in manifests[event]["subtitles"]:
            ...
            if end_ms <= start_ms:
                raise Ac0915LongformBuildError(f"subtitle interval is empty: {event}")
            rows.append(
                {
                    "event": event,
    ...
    if len(rows) != EXPECTED_SUBTITLES or offset_frames != EXPECTED_FRAMES:
        raise Ac0915LongformBuildError("global subtitle dimensions differ")
    return rows
...
def review_path(root: Path, edition: str) -> Path:
    return (
        ...
        / EDITIONS[edition]
        / "story"
        / f"ac0915_{TITLE}_严格无BGM__{edition}.mp4"
    )
...
def validate_probe(value: Mapping[str, Any]) -> dict[str, bool]:
    ...
    checks = {
        ...
        "chapter_count_18": len(value.get("chapters", [])) == EXPECTED_EVENTS,
    }
    if not all(checks.values()):
        raise Ac0915LongformBuildError(f"media QA failed: {checks}")
    return checks
...
def verify_production(
    ...
    if (
        ...
        raise Ac0915LongformBuildError("production manifest differs")
    media = []
    decoded_audio = set()
    for edition in EDITIONS:
        path = review_path(output_root, edition)
        if not path.is_file():
            raise Ac0915LongformBuildError(f"edition is absent: {path}")
        value = probe(
            ffprobe,
            path,
    ...
    if len(decoded_audio) != 1:
        raise Ac0915LongformBuildError("none/JA/ZH decoded audio differs")
    if max(row["duration_seconds"] for row in media) - min(
        row["duration_seconds"] for row in media
    ) > 0.001:
        raise Ac0915LongformBuildError("edition durations differ")
    report = {
        "schema": "magireco-ac0915-exhaustive-production-verification-v1",
        "status": "AUTOMATED_QA_PASSED_HUMAN_PLAYBACK_REQUIRED",
        "content_group_count": 1,
        "edition_file_count": 3,
        "dirinfo_route_coverage": "42/42",
        ...
        "identical_complete_presentation_aliases_removed": 3,
...
def build(args: argparse.Namespace) -> Path:
    ...
    if output_root.exists():
        raise Ac0915LongformBuildError(f"immutable output already exists: {output_root}")
    if args.resume_staging is not None:
        ...
        if (
            ...
            raise Ac0915LongformBuildError("resume staging boundary differs")
    else:
    ...
    try:
        ...
        for event in order:
            ...
            if len(videos) != 1 or int(videos[0].get("nb_read_frames", -1)) != expected:
                raise Ac0915LongformBuildError(f"event frame grid differs: {event}")
        ...
        ja_ass = staging / "manifests" / "ac0915_exhaustive_ja.ass"
        zh_ass = staging / "manifests" / "ac0915_exhaustive_zh.ass"
        ...
        manifest = {
            "schema": "magireco-ac0915-exhaustive-production-manifest-v1",
            "status": "AUTOMATED_QA_PASSED_HUMAN_PLAYBACK_REQUIRED",
            "family": "ac0915",
            "title": TITLE,
            "content_group_count": 1,
        ...
        (staging / "README.md").write_text(
            "# ac0915 exhaustive native-416 no-BGM longform\n\n"
            "One 131.100-second content group preserves all 42 exact DirInfo routes as "
            "18 unique complete event presentations. The three exact AV/subtitle aliases "
            "008=003, 019=005 and 021=007 remain in the manifest but are not replayed. "
            "All 42 unique loadable CRI videos are covered. Runtime-selected counter glyphs "
            "are UI choices proven by IDA, not omitted video identities.\n",
            encoding="utf-8",
        )
        (staging / "HUMAN_REVIEW_CHECKLIST.md").write_text(
            "# 人工验收重点\n\n"
            "1. 本目录只有1个内容组；NONE/JP/ZH是同一长片的三种字幕轨。\n"
            "2. 按18个章节检查顺序自然、无整段重复、无缺失分支。\n"
            "3. 逐句核对15句对白的开口、声音与JA/ZH字幕；新长片未继承人工批准。\n"
            "4. 重点检查南岛特别画面的叠层，以及末段上乘冲击的加法混合。\n"
            "5. 末段动态数字属于游戏实时选择UI；本片穷尽的是42个独立视频源。\n"
            "6. 确认无BGM，同时保留34次已验证对白/SE。\n",
            encoding="utf-8",
        )
        (staging / "UPLOAD_GUIDE.md").write_text(
            "# 上传指南（验收前暂停上传）\n\n"
            "- 内容组：1组；NONE/JP/ZH为同一内容。\n"
            "- 建议分P名：`Magius三人 全告知·胜利·特别画面穷尽合集 ac0915`；"
            "JP追加`__ja`；ZH追加` 中文版`。\n"
...
def main() -> int:
    ...
    if args.verify_existing:
        ...
        print(
            f"PASS_RENDER groups=1 editions=3 routes=42 unique_presentations=18 "
            f"unique_videos=42 frames={EXPECTED_FRAMES} "
