#!/usr/bin/env python3
"""Build the exhaustive native-416 ac1104 editorial longform.

The audience product is one code-reachable, duplicate-free collection of all
17 complete event presentations.  Mutually exclusive DirInfo routes are
...
strict no-BGM partitioning come from the exact ac1104 audio authority.
...
the bounded 46-source media audit pass.
"""
...
REQUIRED_EVENTS = tuple(f"ac1104_{index:03d}" for index in range(1, 18))
EDITORIAL_ORDER = (
    "ac1104_001",  # white short entry
    "ac1104_005",  # red short entry
    "ac1104_006",  # white long entry
    "ac1104_007",  # red long entry
    "ac1104_002",  # common one-on-one setup
    "ac1104_003",  # first three-person outcome
    "ac1104_004",  # alternate three-person outcome
    "ac1104_008",  # common second-stage entry
    "ac1104_009",  # common one-on-one handoff
    "ac1104_014",  # alternate second-stage entry
    "ac1104_015",  # alternate one-on-one handoff
    "ac1104_010",  # standard outcome A
    "ac1104_011",  # standard outcome B setup
    "ac1104_012",  # standard loss
    "ac1104_013",  # standard win
    "ac1104_016",  # revival entry
    "ac1104_017",  # revival completion
)
FRAME_RATE = "30/1"
...
EXPECTED_AUTHORED_CHUNK_OCCURRENCES = 60
EXPECTED_LOADABLE_DGM_IDENTITIES = 46
EXPECTED_UNREACHABLE_ALIAS_NAMES = 4
EXPECTED_EVENT_SOURCE_OCCURRENCES = 64
EXPECTED_RETAINED_AUDIO = 81
EXPECTED_EXCLUDED_BGM = 2
EXPECTED_SUBTITLES = 54
KNOWN_SHORT_LOOP_SOURCE = "ac1104_lev_c020_fukkatu_win_LP"


class Ac1104ExhaustiveError(ValueError):
    pass
...
def validate_authorities(
    ...
    if (
        layers.get("schema")
        != "magireco-ac1104-movielayer-runtime-reachability-authority-v1"
        or layers.get("status") != "passed"
        or int(layer_assertions.get("runtime_events", -1)) != 17
        ...
        raise Ac1104ExhaustiveError("ac1104 MovieLayer authority differs")

    audio_assertions = audio.get("assertions", {})
    audio_decision = audio.get("decision", {})
    if (
        audio.get("schema")
        != "magireco-ac1104-event-audio-runtime-and-sound-bus-authority-v1"
        ...
        != "CLOSED"
        or int(audio_assertions.get("total_audio_occurrences", -1)) != 83
        ...
        raise Ac1104ExhaustiveError("ac1104 audio authority differs")
...
def resolve_routes(rows: Sequence[Mapping[str, str]]) -> list[dict[str, Any]]:
    ...
    if set(grouped) != set(range(EXPECTED_ROUTE_COUNT)):
        raise Ac1104ExhaustiveError("DirInfo kind-56 route index set differs")
    ...
    for row_index in sorted(grouped):
        ...
        if len(events) != len(set(events)) or not set(events) <= set(REQUIRED_EVENTS):
            raise Ac1104ExhaustiveError(f"invalid event sequence in route {row_index}")
        union.update(events)
        routes.append(
            {
                "row_index": row_index,
    ...
    if union != set(REQUIRED_EVENTS):
        raise Ac1104ExhaustiveError("DirInfo route union does not cover 17 events")
    return routes
...
def event_code_map(rows: Sequence[Mapping[str, str]]) -> dict[str, str]:
    ...
    for row in rows:
        ...
        if previous != code:
            raise Ac1104ExhaustiveError(f"DirInfo code differs for {event}")
    if set(result) != set(REQUIRED_EVENTS):
        raise Ac1104ExhaustiveError("DirInfo event-code set differs")
    return result


def source_path_for_dgm(named_video_root: Path, dgm_name: str) -> Path:
    if dgm_name.startswith("ac1104_"):
        return (named_video_root / "main" / "ac1104" / f"{dgm_name}.mp4").resolve()
    if dgm_name.startswith("ac8040_"):
        return (named_video_root / "patch" / "ac8040" / f"{dgm_name}.mp4").resolve()
    raise Ac1104ExhaustiveError(f"unexpected ac1104 DGM family: {dgm_name}")


def layer_role(parent_z2d: str, dgm_name: str) -> str:
    if (
        "_title_" in dgm_name
        or parent_z2d == "ac1104_lev_c009"
        or parent_z2d == "ac8040_shouri_EF_small"
        or parent_z2d == "ac1104_3off_c014_c015_win_rogo"
    ):
...
def _video_stream(probe: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if len(streams) != 1:
        raise Ac1104ExhaustiveError("source does not contain exactly one video stream")
    return streams[0]
...
def build_bounded_source_authority(
    ...
    for chunk in layers["z2d_chunks"]:
        for row in chunk["movie_layers"]:
            ...
            if prior != frames:
                raise Ac1104ExhaustiveError(f"DGM frame interval differs: {name}")
            compiled_indices[name] = int(row["compiled_table_index"])
    if len(expected_frames) != EXPECTED_LOADABLE_DGM_IDENTITIES:
        raise Ac1104ExhaustiveError("loadable DGM identity count differs")
    ...
    for name in sorted(expected_frames):
        ...
        known_loop_shortfall = name == KNOWN_SHORT_LOOP_SOURCE
        if (
            stream.get("codec_name") != "h264"
            or stream.get("r_frame_rate") != FRAME_RATE
            or stream.get("pix_fmt") != "yuv420p"
            or (int(stream.get("width", 0)), int(stream.get("height", 0)))
            not in {(416, 232), (256, 144)}
            or (
                source_frames != expected
                and not (known_loop_shortfall and source_frames == 5 and expected == 23)
            )
        ):
            raise Ac1104ExhaustiveError(f"bounded source signature differs: {name}")
        entry = {
            "dgm_name": name,
            ...
            "source_frame_contract": (
                "short_loop_asset_presented_once_then_final_frame_held"
                if known_loop_shortfall
        ...
        resolved[name] = entry
    report = {
        "schema": "magireco-ac1104-bounded-source-media-authority-v1",
        "status": "PASS",
        ...
        "known_short_loop_source_count": sum(
            row["source_frame_contract"].startswith("short_loop") for row in entries
        ),
        "entries": entries,
...
def event_layer_rows(
    ...
    parent_ranges = layers["runtime_parent_event_ranges"][event]["z2d_parent_ranges"]
    chunks = {str(row["name"]): row for row in layers["z2d_chunks"]}
    visible: list[dict[str, Any]] = []
    unreachable: list[dict[str, Any]] = []
    authored_visual_end = 0
    for parent_name, parent_range in parent_ranges.items():
        chunk = chunks.get(parent_name)
        if chunk is None:
            raise Ac1104ExhaustiveError(f"missing exact Z2D chunk: {parent_name}")
        parent_start = int(parent_range["start_frame"])
        parent_end = int(parent_range["end_frame_inclusive"]) + 1
        if int(chunk["header"]["scene_frame_count"]) != parent_end - parent_start:
            raise Ac1104ExhaustiveError(f"parent range differs from chunk: {event}/{parent_name}")
        ...
        for layer in chunk["movie_layers"]:
            ...
            if source is None:
                raise Ac1104ExhaustiveError(f"loadable source identity missing: {dgm_name}")
            source_end_frame = event_start_frame + int(source["source_frame_count"])
            visible.append(
                {
                    **base,
                    **dict(source),
...
def _speaker_code_from_caption(name: str) -> str:
    ...
    if len(parts) < 4:
        return ""
    token = parts[-2] if parts[-1].isdigit() else ""
    return {"mihu": "mif"}.get(token, token)
...
def build_event_manifest(
    ...
    *,
    ...
    if any(row.get("volume_bus") == "BGM" for row in retained):
        raise Ac1104ExhaustiveError(f"BGM leaked into retained audio: {event}")
    if any((int(row["request_id"]), int(row["sound_id"])) != (229, 554) for row in excluded):
        raise Ac1104ExhaustiveError(f"unexpected excluded audio identity: {event}")
    ...
    return {
        ...
        "quality_gates": {
            ...
            "composition_evidence": "ac1104 exact Z2D MovieLayer/runtime-parent authority",
        ...
        "ac1104_generated_manifest_provenance": {
...
def build_inputs(args: argparse.Namespace) -> dict[str, Path]:
    ...
    try:
        ...
        if event_source_occurrences != EXPECTED_EVENT_SOURCE_OCCURRENCES:
            raise Ac1104ExhaustiveError("event source occurrence count differs")
        if unreachable_occurrences != 10:
            raise Ac1104ExhaustiveError("per-event unreachable alias occurrence count differs")
        if sum(len(row["audio"]) for row in manifests.values()) != EXPECTED_RETAINED_AUDIO:
            raise Ac1104ExhaustiveError("retained audio occurrence total differs")
        if sum(len(row["subtitles"]) for row in manifests.values()) != EXPECTED_SUBTITLES:
            raise Ac1104ExhaustiveError("subtitle occurrence total differs")

        projections: dict[str, str] = {}
        for event in EDITORIAL_ORDER:
            projection = presentation_projection(manifests[event])
            if projection in projections:
                raise Ac1104ExhaustiveError(
                    f"exact complete presentation duplicated: {event}/{projections[projection]}"
                )
            projections[projection] = event
        if set(EDITORIAL_ORDER) != set(REQUIRED_EVENTS) or len(projections) != 17:
            raise Ac1104ExhaustiveError("editorial order is not one-to-one over 17 events")
        ...
        source_series = {
            ...
            "ordering_evidence": (
                "15 DirInfo kind-56 routes reduce to 17 code-reachable complete event "
        ...
        source_series_path = staging / "source_series_catalogs" / "ac1104_exhaustive.json"
        write_json(source_series_path, source_series)

        series = {
            "schema": "magireco-ac1104-exhaustive-editorial-series-v1",
            "series": "ac1104_exhaustive",
            "status": "passed",
            "event_count": 17,
            "title_zh": "香蕉船对决 全入口·全选项·全结局完整合集",
            "event_sequence": list(EDITORIAL_ORDER),
            "ordering": (
                "entry variants -> common setup -> first outcomes -> second entries -> "
                "standard outcomes -> revival outcome"
            ),
            ...
            "family_state": {
                ...
                "known_family_events": 17,
                "ready_family_events": 17,
            ...
            "source_series_manifest": {
                ...
                "evidence": "code-level 15-route/17-event exhaustive presentation authority",
            },
            "human_playback_required": True,
            "publication_approved": False,
        }
        series_path = staging / "series_proposals" / "ac1104_exhaustive.json"
        write_json(series_path, series)

        translation_path = staging / "translations" / "ac1104_exhaustive_zh_v1.json"
        ...
        authority = {
            "schema": "magireco-ac1104-exhaustive-editorial-order-authority-v1",
            ...
            "presentation_universe": {
                "complete_event_presentation_count": 17,
            ...
            "closed_gates": {
                "event_source_universe": "CLOSED_17_EVENTS_46_LOADABLE_DGM_IDENTITIES",
                "missing_movie_layers": "CLOSED_AS_CODE_UNREACHABLE_ALIASES_WITH_LOADABLE_TWINS",
                "event_audio_and_strict_no_bgm": "CLOSED_81_RETAINED_2_BGM_EXCLUDED",
                "child_local_timing": "CLOSED_FOR_ALL_52_REQSOUND_CALLBACKS",
                "legacy_event17_timing_and_omission": "CLOSED_BY_RUNTIME_PARENT_SCENE_AUTHORITY",
                "duplicate_free_editorial_order": "CLOSED_17_OF_17_UNIQUE",
            },
            "decision": {
                "legacy_43_video_inventory_authoritative": False,
        ...
        authority_path = staging / "AC1104_EXHAUSTIVE_EDITORIAL_AUTHORITY.json"
        write_json(authority_path, authority)

        bindings = {
            "schema": "magireco-ac1104-exhaustive-input-bindings-v1",
            "status": "PASS",
            "bindings": [
                _snapshot(path)
                for path in (
        ...
        verification = {
            "schema": "magireco-ac1104-exhaustive-input-verification-v1",
            "status": "PASS_READY_FOR_RENDER",
            "checks": {
                "dirinfo_routes": 15,
                "complete_event_presentations": 17,
                "unique_complete_presentations": 17,
                "loadable_source_identities": 46,
                "loadable_event_source_occurrences": 64,
                "unreachable_add_alias_occurrences": 10,
                "retained_no_bgm_audio_occurrences": 81,
        ...
        (staging / "README.md").write_text(
            "# ac1104 exhaustive authoritative longform inputs\n\n"
            "All 15 DirInfo routes are represented by 17 distinct complete event "
    ...
    return {
        ...
        "series_path": output_root / "series_proposals" / "ac1104_exhaustive.json",
        "translation_path": output_root / "translations" / "ac1104_exhaustive_zh_v1.json",
    }
...
def load_existing_inputs(output_root: Path) -> dict[str, Path]:
    ...
    authority = read_json(output_root / "AC1104_EXHAUSTIVE_EDITORIAL_AUTHORITY.json")
    series_path = output_root / "series_proposals" / "ac1104_exhaustive.json"
    translation_path = output_root / "translations" / "ac1104_exhaustive_zh_v1.json"
    series = read_json(series_path)
    if (
        verification.get("status") != "PASS_READY_FOR_RENDER"
        or verification.get("checks", {}).get("unique_complete_presentations") != 17
        or authority.get("decision", {}).get("new_exhaustive_render_allowed") is not True
        or series.get("event_sequence") != list(EDITORIAL_ORDER)
    ):
        raise Ac1104ExhaustiveError("existing ac1104 input checkpoint differs")
    return {
        "output_root": output_root,
...
def verify_production(production_root: Path, ffprobe: str) -> dict[str, Any]:
    family = production_root / "ac1104_exhaustive_full_no_bgm_editions_v1"
    ...
    if (
        ...
        raise Ac1104ExhaustiveError("produced family manifest/QA differs")
    media: list[dict[str, Any]] = []
    for edition in ("none", "ja", "zh"):
        path = family / "video" / f"ac1104_exhaustive_full_no_bgm_editions_v1__{edition}.mp4"
        probe = _probe_video(path, ffprobe)
        ...
        if (
            ...
            raise Ac1104ExhaustiveError(f"media signature differs: {path}")
        media.append(
            {
                "edition": edition,
    ...
    if len({row["frame_count"] for row in media}) != 1:
        raise Ac1104ExhaustiveError("edition frame grids differ")
    if max(row["duration_seconds"] for row in media) - min(row["duration_seconds"] for row in media) > 0.001:
        raise Ac1104ExhaustiveError("edition container durations differ")
    report = {
...
def render(args: argparse.Namespace, inputs: Mapping[str, Path]) -> dict[str, Any]:
    ...
    command = [
        ...
        "--series-manifest",
        f"ac1104_exhaustive={inputs['series_path']}",
        "--translation-map",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.reuse_inputs:
        ...
        print(
            "PASS routes=15 event_presentations=17 unique_presentations=17 "
            "loadable_sources=46 source_occurrences=64 audio=81 subtitles=54 "
