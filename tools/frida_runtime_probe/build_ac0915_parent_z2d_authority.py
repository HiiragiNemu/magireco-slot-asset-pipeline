"""Extract every exact parent Z2D referenced by the ac0915 runtime scene tree."""
...
EXPECTED_Z2D_OCCURRENCE_COUNT = 48
EXPECTED_UNIQUE_Z2D_COUNT = 39
EXPECTED_EXTRACTABLE_Z2D_COUNT = 34
...
def collect_event_z2d_bindings(
    ...
    if not isinstance(events, dict) or len(events) != EXPECTED_EVENT_COUNT:
        raise AuthorityError(
            f"ac0915 runtime event count differs: {len(events or {})}"
        )
    foreign_events = sorted(
        event_name for event_name in events if not event_name.startswith("ac0915_")
    )
    if foreign_events:
    ...
    if len(all_names) != EXPECTED_Z2D_OCCURRENCE_COUNT:
        raise AuthorityError(
            f"ac0915 parent Z2D occurrence count differs: {len(all_names)}"
        )
    unique_names = sorted(set(all_names))
    if len(unique_names) != EXPECTED_UNIQUE_Z2D_COUNT:
        raise AuthorityError(
            f"ac0915 unique parent Z2D count differs: {len(unique_names)}"
        )
    ...
    *, runtime_path: Path, apk: Path, binary: Path, output_dir: Path
    ...
    try:
        ...
        if len(extractable_names) != EXPECTED_EXTRACTABLE_Z2D_COUNT:
            raise AuthorityError(
                "ac0915 exact archive-backed parent count differs: "
                f"{len(extractable_names)}"
            )
        if len(runtime_only_names) != EXPECTED_RUNTIME_ONLY_NODE_COUNT:
            raise AuthorityError(
                "ac0915 runtime-only type-20 node count differs: "
        ...
        report = {
            "schema": "magireco-ac0915-parent-z2d-event-bindings-v1",
            "status": "PASSED",
        ...
        (stage / "AC0915_PARENT_Z2D_EVENT_BINDINGS.json").write_text(
            json.dumps(report, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        (stage / "README.md").write_text(
            "# ac0915 exact parent Z2D authority\n\n"
            "21 runtime event containers reference 48 type-20 occurrences: "
            "34 unique exact APK Z2D chunks and 5 runtime-only numeric nodes. "
